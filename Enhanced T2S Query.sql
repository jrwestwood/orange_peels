...
WHERE 
  1=1
  AND sac.Version IN (var_version,'public.AOP',var_actual)
...
CASE
    ...
    WHEN COALESCE(IBP6.IBP_DEMAND_PLAN,IBP5.IBP_DEMAND_PLAN) <=0 THEN 0
    WHEN COALESCE(IBP6.IBP_DEMAND_PLAN,IBP5.IBP_DEMAND_PLAN) IS NULL THEN 0
...
-- FIRST AGGREGATION REMOVES T-CODE LEVEL OF DETAIL BUT REMAINS AT THE MATERIAL LEVEL
IBP2TPM_AGG0 AS(
SELECT
CUSTOMERNAME1,
CUSTOMERNAME2,
...
-- NEXT AGGREGATION REMOVES THE MATERIAL, LEAVING THE SUBBRANDVOLUMESIZE / WEEK LEVEL 
IBP2TPM_AGG1 AS(
SELECT
CUSTOMERNAME1,
CUSTOMERNAME2,
...
-- NEXT AGGREGATION REMOVED THE WEEK GRANULARITY, LEAVING THE SUBBRANDVOLUMESIZE / FISCPER_NO LEVEL
IBP2TPM_AGG2 AS (
  SELECT
CUSTOMERNAME1,
CUSTOMERNAME2,
...
-- NEXT AGGREGATION REMOVED THE CUSTOMERLEVEL6 GRANULARITY, THIS CAN BE COMBINED IN THE PREVIOUS AGGREGATION ONCE TESTED TO STREAMLINE THE QUERY
IBP2TPM_AGG3 AS (
  SELECT
CUSTOMERNAME1,
CUSTOMERNAME2,
...
-- SELECT * FROM TPM_AGG
-- WHERE CUSTOMERNAME3 = "METRO NY"
-- AND TPM_SubBrandVolumeSize = "TROPICANA PURE PREMIUM 89 OZ"
-- AND TPM_SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"


-- SELECT * FROM IBP6
-- WHERE CUSTOMERNAME3 = "METRO NY"
-- AND IBP_SubBrandVolumeSize = "TROPICANA PURE PREMIUM 89 OZ"
-- -- AND IBP_SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"
...
-- SELECT * FROM IBP6
-- WHERE CUSTOMERNAME6 = "J&J FARMS CREAMERY CO"
-- AND IBP_CALWEEK = '202536'
-- AND IBP_SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"

-- SELECT * FROM IBP2TPM
-- WHERE CUSTOMERNAME6 = "J&J FARMS CREAMERY CO"
-- AND CALWEEK = '202536'
-- AND SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"

-- SELECT * FROM IBP2TPM
-- WHERE CUSTOMERNAME3 = "METRO NY"
-- AND SubBrandVolumeSize = "TROPICANA PURE PREMIUM 89 OZ"
-- AND SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"

-- SELECT * FROM IBP2TPM_AGG0
-- WHERE CUSTOMERNAME3 = "METRO NY"
-- AND SubBrandVolumeSize = "TROPICANA PURE PREMIUM 89 OZ"
-- AND SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"

-- SELECT * FROM IBP2TPM_AGG1
-- WHERE CUSTOMERNAME3 = "METRO NY"
-- AND SubBrandVolumeSize = "TROPICANA PURE PREMIUM 89 OZ"
-- AND SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"

-- SELECT * FROM IBP2TPM_AGG2
-- WHERE CUSTOMERNAME3 = "METRO NY"
-- AND SubBrandVolumeSize = "TROPICANA PURE PREMIUM 89 OZ"
-- AND SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"

-- SELECT * FROM IBP2TPM_AGG3
-- WHERE CUSTOMERNAME3 = "METRO NY"
-- AND SubBrandVolumeSize = "TROPICANA PURE PREMIUM 89 OZ"
-- AND SubBrandVolumeSize = "NAKED JUICE SMOOTHIE 46 OZ"
...
IBP2TPM_AGG3.FORECAST_TOTAL_SHIPMENT_VOLUME,
  ...
  CASE
    WHEN COALESCE(IBP2TPM_AGG3.SAC_Period,SAC_DIM.Date) >= var_period THEN COALESCE(SAC_PRICING_BOY.LIST_PRICE_BOY5,SAC_PRICING_BOY4.LIST_PRICE_BOY4,LIST_PRICE)*IBP_DEMAND_PLAN
...
WHERE 1=1
AND COALESCE(IBP2TPM_AGG3.CUSTOMERNAME5,SAC_DIM.CUSTOMERNAME5) != 'NATIONAL ACCOUNTS (INDIRECT)'
